...
if model == "slave" then
    ...
    function PERSISTENT_HANDLER.save_user_data(task)
        ...
        local sql = string.format("UPDATE `mud`.`users` SET `name` = '%s', `exp` = '%d', `updated_at` = '%s' WHERE `uid` = %d", userdata.name, userdata.exp, now, uid)
        dbquery(sql)
    ...
    function CMD.create_user_data(uid, name)
        ...
        local sql = string.format("INSERT INTO `mud`.`users` (`uid`, `name`, `exp`, `created_at`, `updated_at`) VALUES ('%d', '%s', '%d', '%s', '%s')", uid, name, 0, now, now)
        dbquery(sql)
    ...
    function CMD.load_user_data(uid)
        ...
        local sql = string.format("SELECT * FROM `mud`.`users` WHERE `uid` = '%d' LIMIT 1", uid)
